...
void runSolvers(const string &name) {
    cout << "--- " << name << " ---\n";

    T cube;
    // manual scramble: F U R
    cube.f(); cube.u(); cube.r();

    {
        ...
        if (!solution.empty()) { cout << "Solution: "; printMoves(solution); }
    }
    ...
    // IDA* requires a pattern database file; try without it (some constructors may accept default)
    try {
        T working = cube;
        ...
        cout << "IDAstarSolver: skipped (db missing or constructor failed)\n";
    }
...
int main() {
    cout << "Manual test: scramble F U R\n\n";
    runSolvers<RubiksCubeBitboard, HashBitboard>("Bitboard");
    runSolvers<RubiksCube3dArray, Hash3d>("3D Array");
    runSolvers<RubiksCube1dArray, Hash1d>("1D Array");
    return 0;
}
